...
dNEP is a research repository for analyzing behavioral and neuroimaging data related to lexicality and prosody processing in native Mandarin speakers and second language (L2) learners.
...
| `dnep_ROI_plots.Rmd` | ROI bar plots for Figures 2b, 3a, 3b, 4, 5, S2, S3 using HCP-MMP parcels |
...
```
dnep_quality_control.py → dnep_ha.py → dnep_ha_glm_wholebrain.py → dnep_ha_roi.py → dnep_ROI_plots.Rmd
```
...
5. `dnep_ROI_plots.Rmd` — reads CSVs from `dnep_ha_roi.py`, generates ROI bar plots for Figures 2b, 3a, 3b, 4, 5, S2, S3

## Dependencies

**Python:** numpy, scipy, pandas, statsmodels, neuroboros, hyperalignment

**R:** tibble, dplyr, tidyr, ggplot2, lme4, lmerTest, emmeans, Rmisc
